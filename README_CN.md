...
本仓库是基于 [Tencent/WeKnora](https://github.com/Tencent/WeKnora) 的个人二次开发项目，在所用上游基线上累计形成 **33 个分支独有提交**。二次开发重点是把自动生成的 Wiki 内容升级为可治理、可审计的产品流程：

- 10 类结构化 Wiki 知识卡，以及 L0/L1 分级审核队列；
- 跨概念冲突检测、证据对照和逐冲突裁决；
- 审核决策、版本历史、重复快照与可追溯反馈信号；
- 通过 MCP 暴露 Wiki Agent 检索能力，并增加 Token 配额与限流。

这是独立的个人作品集分支，并非腾讯官方发行版本。下方保留了上游项目文档和 MIT 许可证信息。
