...
func (r *wikiGovernanceRepository) SubmitFeedbackSignal(ctx context.Context, tenantID uint64, kbID, actor string, in *types.WikiFeedbackSignalInput) (*types.WikiFeedbackSignal, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("knowledge_base_id = ? AND id IN ?", kbID, []string(nodeIDs)).Find(&pages).Error; err != nil {
			return err
		}
		if len(pages) == 0 {
		...
		for i := range pages {
			...
			evidence, _ := json.Marshal(map[string]any{"feedback_signal_id": signal.ID, "feedback_text": in.FeedbackText, "suggested_correction": in.SuggestedCorrection, "related_request_id": in.RelatedRequestID})
